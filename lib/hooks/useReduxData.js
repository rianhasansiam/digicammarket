...
export const useReduxData = ({ entity, enabled = true, params = {} }) => {
  ...
  useEffect(() => {
    if (!config) return;
    if (enabled && !entityState.data?.length && !entityState.isLoading) {
      dispatch(config.fetchAction(params));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [enabled, entity, dispatch, entityState.data?.length, entityState.isLoading]);
  ...
  return {
    ...
    refetch,
    hasData: Boolean(entityState.data && (Array.isArray(entityState.data) ? entityState.data.length > 0 : true)),
    isEmpty: entityState.data ? (Array.isArray(entityState.data) ? entityState.data.length === 0 : false) : true,
