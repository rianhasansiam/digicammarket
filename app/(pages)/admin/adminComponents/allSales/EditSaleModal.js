...
const EditSaleModal = ({ isOpen, onClose, sale, onSuccess, products = [], categories = [] }) => {
  ...
  useEffect(() => {
    if (sale) {
      ...
      setFormData({
        ...
        backgroundColor: sale.backgroundColor || '#dc2626',
        textColor: sale.textColor || '#ffffff',
        accentColor: sale.accentColor || '#fbbf24',
        productIds: sale.productIds || [],
        categoryIds: sale.categoryIds || [],
        bannerImage: sale.bannerImage || '',
        showCountdown: sale.showCountdown !== false,
  ...
  const validateForm = () => {
    ...
    if (formData.discountValue < 0) newErrors.discountValue = 'Discount cannot be negative';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await updateData(saleId, formData);
      onSuccess();
    } catch (error) {
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-white/80 text-sm">Update your promotion details</p>
                </div>
              </div>
              <button
                onClick={onClose}
            ...
            {activeTab === 'appearance' && (
              <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
                    {presetColors.map((preset, index) => (
                      ...
                      >
                        ...
                        >
                          <span className="text-sm font-bold" style={{ color: preset.accent }}>SALE</span>
                        </div>
                ...
                <div>
                  ...
                  <div className="rounded-xl p-6 text-center" style={{ backgroundColor: formData.backgroundColor }}>
                    <div className="text-sm font-medium uppercase tracking-wider mb-2 opacity-80" style={{ color: formData.accentColor }}>
                      {formData.saleType} Sale
                    </div>
                    <h3 className="text-2xl font-bold mb-1" style={{ color: formData.textColor }}>
                      {formData.title || 'Sale Title'}
                    </h3>
                    <p className="text-sm opacity-80" style={{ color: formData.textColor }}>
                      {formData.subtitle || 'Sale subtitle'}
                    </p>
                    {formData.discountValue > 0 && (
                      <div className="inline-block mt-3 px-4 py-2 rounded-full font-bold" style={{ backgroundColor: formData.accentColor, color: formData.backgroundColor }}>
                        ...
                        {products.slice(0, 50).map((product) => (
                          <button
                            ...
                            className={`w-full p-3 text-left transition-colors flex items-center space-x-3 ${
                              formData.productIds.includes(product._id || product.id) ? 'bg-blue-50' : 'hover:bg-gray-50'
                            }`}
                          >
